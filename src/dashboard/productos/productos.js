...
// ===== ELEMENTOS DOM =====
const elementos = {
    filtroNombre: document.getElementById('filtroNombreProducto'),
    filtroSucursal: document.getElementById('filtroSucursalProductos'),
    filtroPrecio: document.getElementById('filtroPrecioProductos'),
    contenedor: document.getElementById('contProductos'),
...
const renderizar = () => {
    ...
    elementos.contenedor.querySelectorAll('[data-action="editar"]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            console.log('Editar producto:', btn.dataset.id);
    ...
    elementos.contenedor.querySelectorAll('[data-action="eliminar"]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (confirm('¿Está seguro de eliminar este producto?')) {
                console.log('Eliminar producto:', btn.dataset.id);
...
const crearItemHTML = (item) => {
    ...
    const clasificacion = item.clasificacion || item.categoria || 'Sin clasificar';
        ...
        <div class="item-row bg-white rounded-lg p-4 border border-gray-200">
            <div class="grid grid-cols-[2fr_1fr_1fr_1fr_1.5fr_auto] gap-4 items-center">
                <div>
                    ...
                    <p class="text-xs text-gray-500">Categoría: ${clasificacion}</p>
                </div>
                ...
                <div class="text-center">
                    ...
                    <p class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm inline-block">${sucursal}</p>
                </div>
    ...
    return `
        <div class="flex items-center justify-center gap-2 mt-6 flex-wrap">
            ...
            <div class="text-sm text-gray-600 ml-4">
                Página ${currentPage} de ${totalPaginas} | ${filteredItems.length} productos
            </div>
        </div>
    `;
};
...
const cargarProductos = async () => {
    ...
    const datos = await obtenerDatos(`${API_BASE}/Producto/obtener_producto_por_filtros`);
